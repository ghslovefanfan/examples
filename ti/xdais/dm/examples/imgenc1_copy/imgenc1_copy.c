...
#define IMGENC1_COPY_VERSIONSTRING "1.00.00.0"
...
extern IALG_Fxns IMGENC1COPY_TI_IALG;
    ...
    switch (id) {
        ...
        case XDM_GETVERSION:
            if ((status->data.buf != NULL) &&
                (status->data.bufSize >= strlen(IMGENC1_COPY_VERSIONSTRING))) {
                strncpy((char *)status->data.buf, IMGENC1_COPY_VERSIONSTRING,
                    strlen(IMGENC1_COPY_VERSIONSTRING));

                /* null terminate the string */
                status->data.buf[strlen(IMGENC1_COPY_VERSIONSTRING)] = '\0';
