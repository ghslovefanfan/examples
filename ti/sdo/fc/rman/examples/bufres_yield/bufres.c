...
static IRES_Handle getBuf(IALG_Handle h, IRES_ProtocolArgs *args, Int scratchId,
        IRES_Status *status);
static IRES_Status freeBuf(IALG_Handle h, IRES_Handle res, IRES_ProtocolArgs *args,
        Int scratchId);
...
extern Void getStaticProps(IRES_Handle res, IRES_Properties *props);
...
static IRES_Status initBuf(IRESMAN_Params *args)
{
    ...
    DBC_require(args->allocFxn != NULL);
    DBC_require(args->freeFxn != NULL);
    ...
    buffer.scratchId = -1;
    buffer.refCount = 0;
        loopcnt = buffer.length >>2;

    for(i = 0; i < loopcnt; i++) {
/*         (UInt32)((UInt8 *)buffer.base)[i * 4] = cArgs->fillValue;  */
        ((UInt32 *)(buffer.base))[i] = cArgs->fillValue;  
    } 
...
/* ARGSUSED */
{
     ...
     *  being used by another algorithm with the same scratchId that, or if
