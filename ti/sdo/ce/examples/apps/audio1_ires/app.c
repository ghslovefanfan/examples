...
#if USE_ANCDATA
    ancBuf = (XDAS_Int8 *)Memory_contigAlloc(ENCANCBUFSIZE, BUFALIGN);
    if (ancBuf == NULL) {
        goto end;
    }
#endif
...
#if USE_ANCDATA
    if (ancBuf) {
        Memory_contigFree(ancBuf, ENCANCBUFSIZE);
    }
#endif
...
/*
 *  @(#) ti.sdo.ce.examples.apps.audio1_ires; 1,0,0,96; 9-9-2008 02:00:20; /db/atree/library/trees/ce-h27x/src/
 */
